...
{
//check id of message thread
if(isset($_GET['id']))
{
$id = intval($_GET['id']);
//fetch data related to that thread
$req1 = mysql_query('select title, user1, user2 from pm where id="'.$id.'" and id2="1"');
$dn1 = mysql_fetch_array($req1);
//check if user is part of thread
if($dn1['user1']==$_SESSION['userid'] or $dn1['user2']==$_SESSION['userid'])
{
	//update the database that user read the message
	mysql_query('update pm set user2read="yes" where id="'.$id.'" and user2 = "'.$_SESSION['userid'].'"');
...
//check if all fields are set of reply
if(isset($_POST['message']) and $_POST['message']!='')
{
	...
	$curtime = time() + (330 * 60);
	//get data of that thread
	if($abc = mysql_query('select user1, user2 from pm where id="'.$id.'" and id2="1"'))
	{
		$abc = mysql_fetch_array($abc);
		if($abc['user1'] == $_SESSION['userid'])
		{
			$user2 = $abc['user2'];
		}
		else
	...
	if(mysql_query('insert into pm (id, id2, title, user1, user2, message, user1read, user2read)values("'.$id.'", "'.(intval(mysql_num_rows($req2))+1).'", "", "'.$_SESSION['userid'].'", "'.$user2.'", "'.$message.'", "yes", "no")'))
	{
...
?>
	<tr>
    	...
    	<td class="left"><div class="date">Date sent: <?php echo $dn2['timestamp']; ?></div>
...
<div class="box_login">
	<form action="login.php" method="post">
		...
		<label for="password">Password</label><input type="password" name="password" maxlength=20 placeholder="password" id="password" /><br /><br/>
        <div class="center">
