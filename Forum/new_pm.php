...
if(isset($_POST['title'], $_POST['recip'], $_POST['message']))
{
	...
	if($_POST['title']!='' and $_POST['recip']!='' and $_POST['message']!='')
	{
		...
		//check if receipent is valid
		if($dn1['recip']==1)
		{
			//check if receipent is different than sender
			if($dn1['recipid']!=$_SESSION['userid'])
			{
				$id = $dn1['npm']+1;
				//make entry into database
				if(mysql_query('insert into pm (id, id2, title, user1, user2, message, user1read, user2read)values("'.$id.'", "1", "'.$title.'", "'.$_SESSION['userid'].'", "'.$dn1['recipid'].'", "'.$message.'", "yes", "no")'))
				{
